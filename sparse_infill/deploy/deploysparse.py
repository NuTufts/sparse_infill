...
def pixelloop(true_t,coord_t,predict_t, input_t,
        ...
        trueadc_h,predictadc_h,diffs2d_h):
    ...
    for i in range(0,true_t.detach().cpu().size()[0]):
        ...
        if (inputval == 0): #dead channel
            ...
            if (trueval > 0):
                diffs2d_h.Fill(trueval,predval)
    ...
    return ADC_img,Input_img,Output_img,Overlay_img,trueadc_h,predictadc_h,diffs2d_h

def main():
    inputfiles = ["/mnt/disk1/nutufts/kmason/data/sparseinfill_data_test.root"]
    outputfile = ["sparseoutput.root"]
    CHECKPOINT_FILE = "../training/test_y_5000.tar"
    ...
    for n in xrange(nentries):
        ...
        ADC_img,Input_img,Output_img,Overlay_img,trueadc_h,predictadc_h,diff2d_h = pixelloop(
                                                    true_t,coord_t,predict_t, input_t,
                                                    ADC_img,Input_img,Output_img,Overlay_img,
                                                    trueadc_h,predictadc_h,diff2d_h)
